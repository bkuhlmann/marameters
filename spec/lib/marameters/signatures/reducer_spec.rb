...
RSpec.describe Marameters::Signatures::Reducer do
  ...
  describe "#call" do
    ...
    it "removes duplicate kind and name" do
      expect(reducer.call([%i[req one], %i[req one]])).to eq([%i[req one]])
    end

    it "removes required keyword if double splat exists" do
      expect(reducer.call([[:keyrest], %i[keyreq one]])).to eq([[:keyrest]])
    end

    it "removes optional keyword if double splat exists" do
      expect(reducer.call([[:keyrest], %i[key one]])).to eq([[:keyrest]])
    end

    it "overrides missing default" do
      expect(reducer.call([%i[key one], %i[key one one]])).to eq([%i[key one one]])
    end

    it "overrides existing default" do
      expect(reducer.call([[:key, :one, 1], %i[key one one]])).to eq([%i[key one one]])
    end
