...
A few constants are provided in case you need to know the kinds of parameters and the categories they are organized in:
...
Marameters::CATEGORIES
# #<data Marameters::Models::Category
#        positionals=[:req, :opt],
#        keywords=[:keyreq, :key],
#        keys=[:keyreq, :key, :keyrest],
#        splats=[:rest, :keyrest],
#        forwards=[:rest, :keyrest, :block]
# >
----

These are also used internally when probing methods or building signatures.
...
probe.empty?                           # false
probe.forwards                         # [:three, :six, :seven]
probe.keys                             # [:four, :five, :six]
...
Marameters.categorize(url.method(:for).parameters, label: "Eaxmple", url: "https://example.com")
...
The signature (`Marameters::Signature`) is the opposite of the probe class which allows you to turn a hash of raw parameters into a method signature. This is most useful when metaprogramming and needing to dynamically build method signatures.
...
[source,ruby]
----
signature = Marameters.signature %i[req one],
                                 %i[opt two],
                                 %i[rest three],
                                 %i[keyreq four],
                                 %i[key five],
...
# Avoid using parameters for proc/lambda defaults. (ArgumentError)

# With lambda (no parameters).
puts Marameters.signature([:key, :demo, -> { Object.new }])
# "demo: Object.new"

# With lambda (with parameters).
puts Marameters.signature([:key, :demo, -> no { no }])
# Avoid using parameters for proc/lambda defaults. (ArgumentError)
----

You can use any primitive, custom object, etc. as a default despite the limited examples shown above.

_Avoid_ using parameters when using procs/lambdas as defaults because only the source (body) of your proc will be used as a _literal_ string when building the method signature in order to ensure lazy evaluation.
...
Use no arguments when you need a bare method signature. Example:
...
When dealing with the complexities of inheritance, there are a few extra methods that can help you. The first is for building the method signature of your subclass based on the parameters of your superclass. Here's a simple example:

[source,ruby]
----
super_params = [%i[req one]]
sub_params = [[:opt, :two, 2]]

Marameters.signature_of(super_params, sub_params).to_s
# "one, two = 2"
----

Use of `+#signature_of+` will allow you to combine the parameters of your super method with the parameters of your current method in order to produce a method signature with no duplicates while ensuring you can forward all necessary parameters that the super method requires.

In addition to building the combined method signature, you'll need to forward all necessary arguments via `super`. This can be done using `#for_super` on the signature. Example:

[source,ruby]
----
Marameters.signature(%i[req one], [:opt, :two, 2], %i[key three]).for_super
# "one, two"
----
