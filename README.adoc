...
[source,ruby]
----
signature = Marameters::Signature.new req: :one,
                                      opt: [:two, 2],
                                      rest: :three,
                                      keyreq: :four,
                                      key: [:five, 5],
...
You'll notice the parameters are a hash _and_ some values are tuples. The reason is it's easier to write a hash than a double nested array as normally produced by the probe or directly from `Method#parameters`. The optional positional and keyword parameters use tuples because you might want to supply a default value using minimal syntax. This can be demonstrated further by using optional keywords (same applies for optional positionals):
